...
description: Use MiniMax Coding Plan API for real-time web search and image understanding (VLM). Uses api.minimax.io with sk-cp-* API keys. No external MCP servers needed.
metadata: {"openclaw":{"emoji":"🧩","requires":{"bins":["node"],"env":["MINIMAX_API_KEY"]},"primaryEnv":"MINIMAX_API_KEY"}}
---

# MiniMax Coding Plan Tool

Uses `api.minimax.io` with **Coding Plan API Key** (format: `sk-cp-*`).  
不需要架設 MCP server，直接用 Node.js 呼叫 API。
