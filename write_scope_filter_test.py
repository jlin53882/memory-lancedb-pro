# -*- coding: utf-8 -*-
path = r'C:\Users\admin\.openclaw\extensions\memory-lancedb-pro\test\smart-extractor-scope-filter.test.mjs'
...
describe('SmartExtractor scopeFilter semantics', () => {
  it('defaults to the target scope when scopeFilter is omitted', async () => {
    ...
    await extractor.extractAndPersist('用戶喜歡烏龍茶。', 'session-1', {
      scope: 'agent:test',
    });
  ...
  it('preserves an explicit undefined scopeFilter for bypass callers', async () => {
    const seen = [];
    const extractor = makeExtractor(seen);
    await extractor.extractAndPersist('用戶喜歡烏龍茶。', 'session-2', {
      scope: 'agent:test',
  ...
  it('preserves an explicit empty scopeFilter array as deny-all', async () => {
    const seen = [];
    const extractor = makeExtractor(seen);
    await extractor.extractAndPersist('用戶喜歡烏龍茶。', 'session-3', {
      scope: 'agent:test',
      scopeFilter: [],
    });
  ...
  it('passes through an explicit non-empty scopeFilter array', async () => {
    const seen = [];
    const extractor = makeExtractor(seen);
    await extractor.extractAndPersist('用戶喜歡烏龍茶。', 'session-4', {
      scope: 'agent:test',
