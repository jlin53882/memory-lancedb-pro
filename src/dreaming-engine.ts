...
/** Top-level durable source field — survives compact/upgrade operations (F2) */
const DREAMING_SOURCE_FIELD = "dreaming-engine";

/** Legacy tag for backward compat (MR2) */
...
async function collectExactScope(
  ...
  while (collected.length < needed) {
    ...
    const ratio = page.length > 0 ? newMatches / page.length : 0;
    if (ratio < LOW_RATIO_THRESHOLD) {
      lowRatioPages++;
      if (lowRatioPages >= MAX_LOW_RATIO_PAGES) {
        debugLog(`paginate [${scope}]: stopping after ${MAX_LOW_RATIO_PAGES} consecutive pages with <${Math.round(LOW_RATIO_THRESHOLD * 100)}% target-scope ratio`);
        break;
      }
    } else {
      lowRatioPages = 0;
