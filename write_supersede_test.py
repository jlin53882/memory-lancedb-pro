# -*- coding: utf-8 -*-
"""
Rewrite supersede-existing-found-bulk.test.mjs to use real SmartExtractor via jiti.

Tests the actual handleSupersede batch mode fix in src/smart-extractor.ts.
"""
...
describe("Issue #676: handleSupersede batch mode with real SmartExtractor", () => {
  ...
  it("store.update() receives metadata with invalidated_at", async () => {
    ...
    assert.strictEqual(updatedMeta.superseded_by, null,
      "superseded_by is null in batch mode (new ID unknown until bulkStore)");
...
with open(r'C:\Users\admin\.openclaw\extensions\memory-lancedb-pro\test\supersede-existing-found-bulk.test.mjs', 'w', encoding='utf-8') as f:
    f.write(content)
print('Done')
