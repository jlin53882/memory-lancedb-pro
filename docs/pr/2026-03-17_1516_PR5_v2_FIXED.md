...
# 響應式調整函式
def _get_column_width(page: ft.Page) -> int | None:
    """根據視窗寬度回傳 Column 寬度"""
...
class CacheQueryView(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        # ✅ 修正：使用 subscribe 模式，避免覆蓋其他 View 的 resize 處理
        self.page.on_resize.subscribe(self._on_window_resize)
...
> ⚠️ 統一格式：所有 `dims` 欄位使用 dict 格式，與 `VIEW_DIMENSIONS` 保持一致
...
VIEW_REGISTRY = {
    ...
    'cache_overview': {
        ...
        'label': '快取總覽',
        'dims': {'width': 1360, 'height': 940},  # ✅ 統一 dict 格式
    },
    'cache_query': {
        'view': CacheQueryView,
        'icon': ft.Icons.SEARCH,
        'label': '快取查詢',
        'dims': {'width': 1400, 'height': 950},   # ✅ 統一 dict 格式
    },
    'cache_shard': {
        'view': CacheShardView,
        'icon': ft.Icons.EDIT,
        'label': '快取分片',
        'dims': {'width': 1500, 'height': 1000},  # ✅ 統一 dict 格式
    },
}
```
...
### Phase 5: 清理
- [ ] 刪除舊 cache_view.py
- [ ] ✅ 新增：清理 `app/views/__init__.py` 中的 `sys.modules` 相容層（如果有對 `app.views.cache_view` 的映射需移除）
- [ ] 執行全部測試
