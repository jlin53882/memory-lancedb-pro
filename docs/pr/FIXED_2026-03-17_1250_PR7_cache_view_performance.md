...
> ⚠️ 根據記憶庫驗證：Flet 0.28.3 不支援 `ListView.builder()` 的 `item_count` / `item_builder` 參數（這是新版 API，專案使用 0.28.3）。只能使用手動分頁載入。
...
def _load_page(self, page: int):
    ...
    for item in items[start:start + PAGE_SIZE]:
        self.type_list.controls.append(self._build_item(item))
    self.page.update()
```
